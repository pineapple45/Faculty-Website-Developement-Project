...
//This is for initializing summernote text-editor
$(document).ready(function() {

  $('.summernote').summernote({
                    height:300,
                    dialogsFade: true,
                    placeholder:"Write details over here..."
                    });

  $(".btn").css({"color": 'white'});
  $(".note-toolbar").css({"background-color": 'black'});
  $(".modal-title").css("position","absolute");
  $(".modal-title").css("float","left");

  $(".note-palette .btn").css("backgroundColor","black");
  $(".note-dropdown-menu").css("backgroundColor","#ffa41b");
  $(".note-children-container").css("backgroundColor","#ffa41b");

  // const editIconBtn = document.querySelectorAll('.edit-icon-btn');
  // editIconBtn.forEach((item, i) => {
  //   item.addEventListener('click', () => {
  //     const modalDetails = document.querySelector('.modal-details');
  //     modalDetails.setAttribute('id','summernote');
  //     console.log(modalDetails);
  //
  //     const addDetails = document.querySelector('.add-details');
  //     addDetails.setAttribute('id','');
